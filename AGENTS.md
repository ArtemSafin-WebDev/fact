...
- Для карточек не использовать `min-height`; предпочитать `aspect-ratio`.
- Антипаттерн: не использовать приём с `gap: 1px` + фоном/паддингом у общего контейнера для имитации разделителей в сетках. Для разделителей использовать бордеры у самих элементов.
