const reviewsPageData = {
  "/reviews.html": {
    title: "Отзывы",
    reviewsPage: {
      breadcrumbs: [
        {
          title: "Главная",
          href: "/",
        },
        {
          title: "О клинике",
          href: "#",
        },
        {
          title: "Отзывы",
          isCurrent: true,
        },
      ],
      titleLines: ["держим безупречный", "рейтинг более 8 лет"],
      ratings: [
        {
          amount: "5.0",
          logo: "/images/ratings/1.webp",
          logoAlt: "Продокторов",
        },
        {
          amount: "4.9",
          logo: "/images/ratings/2.webp",
          logoAlt: "ПоОтзывы",
        },
        {
          amount: "5.0",
          logo: "/images/ratings/3.webp",
          logoAlt: "Яндекс",
        },
        {
          amount: "4.7",
          logo: "/images/ratings/4.webp",
          logoAlt: "2ГИС",
        },
        {
          amount: "4.9",
          logo: "/images/ratings/2.webp",
          logoAlt: "ПоОтзывы",
        },
      ],
      patientsTitle: "Пациенты о нас",
      patientsSubtitle: "Отзывы",
      filters: [
        {
          id: "all",
          title: "Все отзывы",
          active: true,
        },
        {
          id: "laser",
          title: "Лазерная коррекция",
        },
        {
          id: "cataract",
          title: "Катаракта",
        },
        {
          id: "children",
          title: "Детское отделение",
        },
      ],
      videoReviews: [
        {
          href: "https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4",
          fancybox: "video-reviews",
          videoType: "html5video",
          videoWidth: "960",
          videoHeight: "540",
          category: "laser",
          image: "/images/patients/figma-1.webp",
          name: "Сергей",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
        },
        {
          href: "https://media.w3.org/2010/05/sintel/trailer.mp4",
          fancybox: "video-reviews",
          videoType: "html5video",
          videoWidth: "960",
          videoHeight: "540",
          category: "cataract",
          image: "/images/patients/figma-2.webp",
          name: "Сергей",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
        },
        {
          href: "#",
          category: "children",
          image: "/images/patients/figma-3.webp",
          name: "Сергей",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
        },
        {
          href: "#",
          category: "laser",
          image: "/images/patients/figma-4.webp",
          name: "Сергей",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
        },
      ],
      externalTitle: "Отзывы с независимых площадок",
      externalReviews: [
        {
          name: "Марина",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
          href: "#",
          buttonText: "читать полностью",
        },
        {
          name: "Марина",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
          href: "#",
          buttonText: "читать полностью",
        },
        {
          name: "Марина",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
          href: "#",
          buttonText: "читать полностью",
        },
        {
          name: "Марина",
          date: "18 марта 2026",
          text: "Были в клинике с сыном. Остались очень довольны. Особенно хотелось бы отметить администратора Ангелину. Очень спокойная.",
          href: "#",
          buttonText: "читать полностью",
        },
      ],
      showMoreText: "показать еще",
    },
  },
};

export default reviewsPageData;
